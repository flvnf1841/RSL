...
def run_pplm_example(
    ...
    model_checkpoint = 'Dec29_15-56-09_iclserver-desktop_gpt2-medium/'
    ...
    if uncond:
        ...
        "I am social. <skeleton> I am spending time with __ what are you up to? <speaker2> hi, how are you doing today? <speaker1>",
        "I want to have fun with them. <skeleton> I am spending time with __ what are you up to? <speaker2> hi, how are you doing today? <speaker1>",
        "I get yelled at. <skeleton> I am spending time with __ what are you up to? <speaker2> hi, how are you doing today? <speaker1>"]
        skeletons = [
        # None,
        # None,
        # None,
    ...
    with open('personachat_self_original_final2.json') as f:
        ...
        pc_valid = pc["valid"]
        print(len(pc_train))
        split1 = int(len(pc_train)/3) #5959
        split2 = int((2*len(pc_train))/3) #11918

        # ~2000, ~4000, ~5959
        pc_train = pc["train"][2000:4000]
        # tempjson["train"] = pc_train

        tempjson["valid"] = pc_valid
        # print(tempjson)
        # print(len(pc_valid))

        #17878
        #0~1699, 1700~3399, 3400~5099, 5100~6799, 6800~ 8499, 8500 ~ 10199, 10200 ~ 11899, 11900 ~ 13599, 13600 ~ 15199, 15200 ~ 17877
        #1000
        # print(len(pc_valid))
    ...
    #         if skeleton_real == '<none>':
    #             continue

    #         counter+=1
    #         for k in range(0,len(pc_train[i]["utterances"][j]["used_exp_persona"])):

    #             if 'none' in pc_train[i]["utterances"][j]["used_exp_persona"][k]:
    #                 continue

    #             temp = []
    #             history = pc_train[i]["utterances"][j]["history"][-1]

    #             raw_text = pc_train[i]["utterances"][j]["used_exp_persona"][k] + " <skeleton>" + skeleton_real + " <speaker2> " + history + " <speaker1>"
    ...
    #                     (tokenized_cond_text, pert_gen_tok_text, unpert_gen_tok_text)
    #                 )
    #             pc_train[i]["utterances"][j]["new_responses"].append((temp[0], pc_train[i]["utterances"][j]["used_exp_persona"][k]))
    ...
    for i in range(0, len(pc_valid)):
        ...
        for j in range(0, len(pc_valid[i]["utterances"])):
            ...
            counter+=1
            for k in range(0,len(pc_valid[i]["utterances"][j]["used_exp_persona"])):
    
                if 'none' in pc_valid[i]["utterances"][j]["used_exp_persona"][k]:
                    continue
    
                temp = []
                history = pc_valid[i]["utterances"][j]["history"][-1]
    
                raw_text = pc_valid[i]["utterances"][j]["used_exp_persona"][k] + " <skeleton>" + skeleton_real + " <speaker2> " + history + " <speaker1>"
                ...
                pc_valid[i]["utterances"][j]["new_responses"].append((temp[0], pc_valid[i]["utterances"][j]["used_exp_persona"][k]))
    ...
    #     # json.dump(pc,f)
    with open('perturb3_final_valid.json', 'w') as f:
